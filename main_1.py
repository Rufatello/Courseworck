...
"""решил сделать, чтобы пользователь сам вводил компании которые ему интересны переменная companies """
"""Если принципиально,чтобы было как в ТЗ, могу переделать чутка"""
...
def load_company():
    ...
    for i in companies:
        count+=1
        response = requests.get(f'https://api.hh.ru/employers/{i}', params={'per_page': 1, 'only_with_vacancies': True}).json()
...
list_company, url_company = load_company()
    ...
    with conn:
        with conn.cursor() as cur:
            ...
            for i in list_company:
                count_1+=1
                cur.execute('INSERT INTO companies VALUES (%s,%s) ON CONFLICT (company_id) DO NOTHING',(count_1, i['name']))
                conn.commit()
